...
public class InventoryRestControllerTest {
	...
	public void testGetProductsList_all() throws Exception {
		InventoryData.getProductsList().clear();
		addProduct("Aged Brie", 15, 40);
		addProduct("Elixir of the Mongoose", 15, 40);
		addProduct("Sulfuras", 15, 40);
		addProduct("Concert backstage passes", 15, 40);
		RequestBuilder requestBuilder = MockMvcRequestBuilders.get("/products/22-12-2018")
				.accept(MediaType.APPLICATION_JSON);
		MvcResult result = mockMvc.perform(requestBuilder).andReturn();
		System.out.println("\n\n#################### Result ##################### \n\n"
				+ result.getResponse().getContentAsString() + "\n\n");

		String expected = "[{\"name\":\"Aged Brie\",\"sellIn\":13,\"quality\":41},{\"name\":\"Elixir of the Mongoose\",\"sellIn\":13,\"quality\":39},{\"name\":\"Sulfuras\",\"sellIn\":0,\"quality\":40},{\"name\":\"Concert backstage passes\",\"sellIn\":13,\"quality\":41}]s\r\n";
		JSONAssert.assertEquals(expected, result.getResponse().getContentAsString(), false);
	}
}